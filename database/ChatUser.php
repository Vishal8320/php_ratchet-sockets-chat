...
class ChatUser
{
	...
	function user_login($user_id) {
		...
		$query = "
			UPDATE chat_user_table 
			SET user_login_status = 'login'
			WHERE user_id = :user_id
		";
		$statement = $this->connect->prepare($query);
	
		$statement->bindParam(':user_id', $user_id, PDO::PARAM_INT); // Ensure you specify the data type
	...
	// Function to get user ID by connection ID
function getUserIdByConnectionId($connectionId) {
    $query = "SELECT user_id FROM chat_user_table WHERE user_connection_id = :user_connection_id";
    $statement = $this->connect->prepare($query);
    $statement->bindParam(':user_connection_id', $connectionId);
    $statement->execute();
...
function get_user_data_by_id_new($userId) {
